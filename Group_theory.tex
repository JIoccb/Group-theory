...
\begin{document}
	...
	\begin{exmpls}
		...
		\begin{enumerate}
			\setlength\itemsep{0.1em}
			\item ${\integers ^ + < \rationals ^ + < \real ^ + < \complex ^ +}$
	...
	\end{consequence}
	\begin{proof}
		Достаточно применить следствие 3 к группе $\integers_p ^ {\times} \, \backslash \ \{0\}$
	\end{proof}
	\begin{consequence}
	...
	\end{proof}
	\begin{exmpl*}
		Пусть ${G = \real ^ +}$ и ${H = \integers ^ +}$. Рассмотрим группу ${F = \complex^{\times} \, \backslash \, \{0\}}$ и гомоморфизм ${\phi: G \rightarrow F}, {\quad g \mapsto e^{2\pi i g} = \cos(2\pi g) + i \sin(2\pi g).}$
	...
	\newpage
	\section{Прямое произведение групп. Теорема о строении конечных абелевых групп}
	\setcounter{definition}{0}
	\begin{definition}
	...
	\begin{remark}
		...
		\begin{enumerate}
			\setlength\itemsep{0.1em}
			\item Из того, что ${\forall a \in A \  \forall m \in \integers{:} \ ma = 0 \Longleftrightarrow m \divby \ord(a),}$ определение экспоненты можно переписать в виде ${\exp A \deq \NOK \{\ord(a) \ | \ a \in A\}.}$
	...
	\newpage
        ...
        \section{Действия групп. Формула Бёрнсайда. Теоремы Силова}
        \setcounter{definition}{0}
        \begin{definition}
        ...
        \end{remark}
        \begin{definition}
            \textit{Орбитой} точки ${x \in X}$ называется множество $Orb(x) \deq \{gx \ | \ g \in G\} \subseteq X$.
        \end{definition}
        \begin{definition}
            \textit{Стабилизатором (стационарной подгруппой, подгруппой изотропии)} точки ${x \in X}$ называется множество $St(x) \deq \{g \in G \ | \ gx = x\} \subseteq G.$
        \end{definition}
        \begin{remark}
            $St(x) < G.$
        \end{remark}
        ...
        \end{definition}
        \begin{definition}
            \textit{Ядром неэффективности действия} $a$ назывется множество $\ker \alpha \deq \{g \in G \ | \ \forall x \in X{:} \ gx = x\}.$
        \end{definition}
        \begin{definition}
	...
	\end{remark}
	\begin{definition}
		Кольцо называется \textit{телом}, если оно ассоциативно, содержит ${1 \neq 0,}$ и любой ненулевой элемент обратим.
        \begin{exmpl*}
